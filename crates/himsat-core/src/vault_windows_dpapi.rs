...
//!
//! The adapter protects one VRK with `Scope::User` only and stores only DPAPI
//! ciphertext in a Himsat-owned ACL-restricted directory. Provider-visible file
//! names contain only an opaque random protector identifier. Owner, vault,
//! generation, requested policy, and VRK bytes remain inside the DPAPI payload.
//!
//! This adapter deliberately reports only `SAME_USER_ACCOUNT`, no per-unlock
//! user-presence requirement, and unknown hardware backing. Machine-scope DPAPI,
//! freshness anchors, protector replacement/rotation, and stronger Windows
//! Hello/CNG claims remain outside B403.
...
impl WindowsProtectorId {
    ...
    /// Returns the opaque bytes for Himsat-owned configuration persistence.
    #[must_use]
    pub const fn as_bytes(&self) -> &[u8; PROTECTOR_ID_BYTES] {
        &self.0
    }
    /// Returns the lowercase hexadecimal ciphertext filename stem.
    #[must_use]
...
impl WindowsDpapiConfig {
    /// Creates one absolute Himsat-owned storage configuration.
    pub fn new(
    ...
    /// Root directory that contains only opaque DPAPI ciphertext files.
    #[must_use]
    pub fn storage_root(&self) -> &Path {
        &self.storage_root
    }
    /// Opaque protector identifier used by this adapter.
    #[must_use]
...
impl WindowsDpapiProtector {
    /// Creates an unconfigured conservative Windows adapter.
    #[must_use]
    ...
    /// Returns the immutable storage configuration used by this adapter.
    #[must_use]
    ...
    fn decrypt_record(&self) -> Result<Zeroizing<Vec<u8>>, ProtectorError> {
        ...
        let plaintext = decrypt_data(&ciphertext, Scope::User, Some(&entropy))
...
impl SecretProtector for WindowsDpapiProtector {
    ...
    fn protect_or_store_vrk(
        ...
        self.verify_storage_root()?;
        if self.config.record_path().exists() {
...
fn current_user_sid() -> Result<(Vec<u8>, String), ProtectorError> {
    let user = current_user().ok_or(ProtectorError::Unavailable)?;
    let domain = std::env::var("USERDOMAIN").map_err(|_| ProtectorError::Unavailable)?;
    if user.is_empty() || domain.is_empty() {
        return Err(ProtectorError::Unavailable);
    }
    let account = format!(r"{domain}\{user}");
    let sid = name_to_sid(&account, None).map_err(map_acl_error)?;
    let sid_string = sid_to_string(sid.as_ptr() as *mut _).map_err(map_acl_error)?;
    Ok((sid, sid_string))
}
