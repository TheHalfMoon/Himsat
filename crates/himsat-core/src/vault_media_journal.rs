//! Append-only session journal for Specification 005B.
//!
//! This module executes only the 005B session-journal codec and fsync
//! discipline. It defines fixed session open/close markers and chunk-commit
//! records, appends them to one caller-owned file per session with a
//! `sync_all` after every record, and replays the file with truncation-safe
//! tail handling: replay returns the exact valid prefix plus a machine
//! `TailStatus` and never half-applies a torn record.
//!
...
//!
//! Deliberately out of scope (005C/005D own them):
//!
//! - recovery reconciliation against the manifest inventory (orphan,
//!   duplication, and commit-count resolution);
//! - fault-injection harnesses and bounded-loss quantification;
//! - nonce reservation integration: commit records quote the 24-byte nonce
//!   so a later reservation log can be built on this file, but this module
//!   neither generates nor reserves nonces;
//! - journal deletion or torn-tail truncation: replay is read-only and
//!   resume refuses a torn tail; mutation of a damaged file is a 005C
//!   recovery decision, never a silent 005B repair.
//!
//! Integrity model, stated honestly: the per-record SHA-256 is
//! crash-tear detection, not a security boundary. It catches torn writes,
//! truncation, and bit rot at replay. Forgery resistance for chunk bytes
//! comes from the 005A AEAD envelope at recovery time, and the journal
//! carries only public 005A header fields plus digests. No secret, key, or
//! plaintext audio ever enters a journal record.
//!
//! Durability model, stated honestly: durability is exactly the host `sync_all`
//! (fsync-class) guarantee per record. This module makes no `F_FULLFSYNC`,
//! directory-entry-durability, or power-loss-proof claim beyond a successful
//! `sync_all` return. One file holds exactly one session; the caller owns
//! file placement and opaque naming per the 004 metadata rules.
//!
//! Donor posture: Meetily recording-manager/journaling ideas, Anarlog
//! community recorder paths, and OpenSuperWhisper file-queue patterns are
//! recorded planning inputs. No donor journal implements the B202-compatible
//! AEAD binding and truncation discipline this file requires, and donor
//! file-queue UX belongs to the 006+ capture surfaces, so 005B adopts no
//! donor code and implements the smallest Himsat-native append-only
//! length-prefixed integrity log instead.
...
/// Fail-closed journal errors for mutating operations. Replay never fails on
/// file content: content anomalies surface as `TailStatus`, and only I/O
...
fn parse_records(bytes: &[u8]) -> (Vec<JournalRecord>, TailStatus) {
    ...
    loop {
        ...
        if bytes.len() < offset + JOURNAL_HEADER_BYTES + payload_len {
            return (
                records,
                TailStatus::TornTail {
                    valid_bytes: offset,
        ...
        // Slice from the checked record_end so no second unchecked copy of
        // the length arithmetic can drift from the bounds just proven.
        // record_end >= offset + HEADER + INTEGRITY always, so the
        // subtractions below cannot underflow.
...
mod tests {
    use super::{
        JournalRecordKind, JournalWriter, TailReason, TailStatus, envelope_digest, replay_journal,
    };
    use crate::vault::{KeyGeneration, VaultId};
    use std::fs;
    ...
    /// Unique journal path removed on drop, so assertion failures cannot
    /// litter the temp dir and files vanish even mid-test. Declared before
    /// any writer handle, it drops after handles close (required on
    ...
    #[test]
    fn open_commit_close_round_trip_is_clean() {
        let file = TestFile::new();
        let path = file.path().to_path_buf();
        let bytes = write_session(&path, 3, true);
        // open(101) + 3 x commit(173) + close(101) = 721 bytes.
        assert_eq!(bytes.len(), 101 + 3 * 173 + 101);
        let replay = replay_journal(&path).expect("replay");
    ...
    fn every_truncation_offset_returns_the_exact_valid_prefix() {
        let file = TestFile::new();
        let path = file.path().to_path_buf();
        let bytes = write_session(&path, 2, true);
        // Record boundaries: open ends at 101, commits at 274 and 447,
        // close at 548. Every truncation must yield exactly the records that
        // complete before the cut and never a partial record.
        let boundaries = [0usize, 101, 274, 447, 548];
        for cut in 0..=bytes.len() {
            fs::write(&path, &bytes[..cut]).expect("truncate fixture");
            let replay = replay_journal(&path).expect("replay");
            // Exact rule: exactly the records fully contained in the cut.
            let contained = [101, 274, 447, 548].iter().filter(|b| **b <= cut).count();
            assert_eq!(
                replay.records().len(),
                contained,
                "cut at {cut} must replay exactly the contained prefix"
            );
            // A cut exactly on a record boundary is a clean crash point:
            // every fsync-completed record stands, nothing is torn.
            if boundaries.contains(&cut) {
                assert_eq!(
                    replay.tail(),
                    TailStatus::CleanEof,
                    "cut at {cut} ends on a record boundary and is clean"
                );
            } else {
                assert!(
                    matches!(replay.tail(), TailStatus::TornTail { valid_bytes, .. } if valid_bytes
                        == boundaries.iter().copied().filter(|b| *b <= cut).max().unwrap_or(0)),
                    "cut at {cut} must stop at the last complete record"
                );
            }
        }
    }
    ...
    #[test]
    fn single_bit_flips_are_detected_at_the_damaged_record() {
        let file = TestFile::new();
        let path = file.path().to_path_buf();
        let bytes = write_session(&path, 2, false);
        // Flip every bit of the second commit (bytes 274..447): replay must
        // keep the open marker plus first commit and stop exactly at 274.
        // Parser order decides the header-region reason (magic framing,
        // version gate, sequence rule, or length/integrity checks); flips at
        // or after the payload start (offset 335) always break the SHA-256
        // integrity with the header still parsing. The stop offset and the
        // preserved prefix are exact for every flip.
        for byte in 274..447 {
            for bit in 0..8 {
                let mut damaged = bytes.clone();
                damaged[byte] ^= 1 << bit;
                fs::write(&path, &damaged).expect("damage fixture");
                let replay = replay_journal(&path).expect("replay");
                assert_eq!(
                    replay.records().len(),
                    2,
                    "flip at byte {byte} bit {bit} must preserve the valid prefix"
                );
                if byte < 335 {
                    assert!(
                        matches!(
                        ...
                        "flip at byte {byte} bit {bit} must stop at offset 274, got {:?}",
                        replay.tail()
                    );
                } else {
                    assert_eq!(
                        ...
                        "flip at byte {byte} bit {bit} must break integrity at 274"
                    );
                }
            }
        }
    ...
    fn magic_corruption_at_offset_zero_yields_empty_prefix() {
        let file = TestFile::new();
        let path = file.path().to_path_buf();
        let mut bytes = write_session(&path, 1, false);
        bytes[0] ^= 0xFF;
        fs::write(&path, &bytes).expect("damage fixture");
        let replay = replay_journal(&path).expect("replay");
        assert!(replay.records().is_empty());
        assert!(
            matches!(
            ...
            "bad magic must stop at offset 0, got {:?}",
            replay.tail()
        );
    }
    ...
    fn sequence_gap_stops_replay_without_applying_later_records() {
        ...
        let path = file.path().to_path_buf();
        let bytes = write_session(&path, 2, false);
        // Splice out the middle commit: open (seq 1) followed by the second
        // commit (seq 3). Both records keep valid integrity, so the parser
        // reaches the sequence rule and stops with SequenceGap.
        let mut gapped = bytes[0..101].to_vec();
        gapped.extend_from_slice(&bytes[274..447]);
        fs::write(&path, &gapped).expect("gap fixture");
        let replay = replay_journal(&path).expect("replay");
    ...
    #[test]
    fn create_refuses_nonempty_file_and_double_open() {
        let file = TestFile::new();
        let path = file.path().to_path_buf();
        let _writer =
            JournalWriter::create(&path, vault(), TEST_SESSION, generation()).expect("create");
        let err = JournalWriter::create(&path, vault(), TEST_SESSION, generation())
            .expect_err("second create must fail");
        assert!(matches!(err, super::JournalError::JournalNotEmpty));
    }
    ...
    fn missing_file_replay_is_an_io_error_not_a_tail() {
        let file = TestFile::new();
        let path = file.path().to_path_buf();
        let err = replay_journal(&path).expect_err("missing file must fail");
    ...
    fn transplanted_vault_binding_stops_replay() {
        ...
        // the transplant must stop with ContextMismatch, never apply.
        ...
        fs::write(&path, &spliced).expect("transplant fixture");
        let replay = replay_journal(&path).expect("replay");
    ...
    fn envelope_digest_is_stable_and_input_sensitive() {
        ...
        let b = envelope_digest(b"chunk-a");
        let c = envelope_digest(b"chunk-b");
        assert_eq!(a, b);
        assert_ne!(a, c);
        assert_eq!(a.len(), 32);
    }
    ...
    fn error_display_names_each_failure_class() {
        let cases: Vec<super::JournalError> = vec![
            super::JournalError::JournalNotEmpty,
            super::JournalError::EmptyJournal,
            super::JournalError::SessionClosed,
            super::JournalError::ContextMismatch,
        ...
        assert!(
            !format!(
                "{}",
                super::JournalError::Io(std::io::Error::other("probe"))
            )
            .is_empty()
        );
    }
