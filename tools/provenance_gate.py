...
def run_legacy_self_test(root: Path, registry: dict[str, Any]) -> None:
    with tempfile.TemporaryDirectory() as temp:
        ...
        target.write_text(json.dumps(legacy_registry, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        try:
...
def self_test(root: Path) -> None:
    ...
    run_legacy_self_test(root, registry)
