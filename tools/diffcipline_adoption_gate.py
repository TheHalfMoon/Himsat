...
for explicitly bounded historical/provider, B401, B403A, B403C, B403D, B404, or B501D candidate transitions. The B401, B403A, B403C, B403D, B404, and B501D exceptions are available only when this gate is executed
...
B501D_PRECONDITION_BASE = "c9493b3705f2cc69493b1b301d04074197b182cf"
B501D_TRUSTED_BASE_DELTA = {"tools/diffcipline_adoption_gate.py"}
B501D_EXPECTED_ADDED_LINES = 22
B501D_EXPECTED_DELETED_LINES = 0
B501D_EXPECTED_BLOBS = {
    "Cargo.lock": "007428e2cd75486b905f93f12446c727b34144a8",
    "crates/himsat-core/Cargo.toml": "68bf6daf53a27bbec780c1aaeb841af6f245d8a3",
    "specs/004-vault-key-crypto/b501d-apple-core-foundation-exposure-evidence.md": "2de7926f71fb65a97e9798f3aeccb0a7ee8b13ab",
    "tools/004p_dependency_closure.py": "bae9d2b89c76aff4ad60420570fcfb6ff91c25cf",
}
B501D_EXPECTED_FILES = set(B501D_EXPECTED_BLOBS)
...
def b501d_trusted_base(base: str) -> bool:
    ...
    B501C gate-hardening merge. Push-triggered qualification after the guarded
    B501D dependency-exposure merge accepts that same trusted gate merge as the
    first parent of current canonical main. The trusted gate merge itself must
    have the exact B501B canonical merge as first parent and change only this
    gate path.
    """
    ...
    try:
        ...
        if len(base_parent_row) != 3 or base_parent_row[1] != B501D_PRECONDITION_BASE:
            return False
        raw = git("diff", "--no-renames", "--name-only", B501D_PRECONDITION_BASE, base)
    except subprocess.CalledProcessError:
        return False
    paths = [line for line in raw.splitlines() if line]
    return set(paths) == B501D_TRUSTED_BASE_DELTA and len(paths) == len(B501D_TRUSTED_BASE_DELTA)


def check_b501d_exception(args: argparse.Namespace, proof: dict[str, Any]) -> int:
    """Accept only the pinned B501D Apple Core Foundation direct exposure."""

    if args.exit_code != 1 or proof.get("verdict") != "REVIEW":
        return fail(
            f"unexpected B501D non-PASS result: exit={args.exit_code} verdict={proof.get('verdict')}"
        )

    try:
        actual_files, actual_added, actual_deleted = actual_git_diff(args.base)
    except (subprocess.CalledProcessError, ValueError) as exc:
        return fail(f"cannot reconcile actual B501D Git diff: {exc}")

    if set(actual_files) != B501D_EXPECTED_FILES or len(actual_files) != len(B501D_EXPECTED_FILES):
        return fail("actual B501D Git changed-path set is not exact")

    files = proof.get("files")
    if not isinstance(files, list) or files != actual_files:
        return fail("B501D Diffcipline proof path list does not equal the actual Git diff")
    if set(files) != B501D_EXPECTED_FILES or len(files) != len(B501D_EXPECTED_FILES):
        return fail("B501D exception changed-path set is not exact")

    if proof.get("changed_files") != len(B501D_EXPECTED_FILES):
        return fail("B501D Diffcipline changed-file count is not exact")
    if actual_added != B501D_EXPECTED_ADDED_LINES or proof.get("added_lines") != actual_added:
        return fail("B501D added-line count is not exact")
    if actual_deleted != B501D_EXPECTED_DELETED_LINES or proof.get("deleted_lines") != actual_deleted:
        return fail("B501D deleted-line count is not exact")
    ...
    if not isinstance(reasons, list) or set(reasons) != expected_reasons or len(reasons) != 2:
        return fail(f"unexpected B501D Diffcipline reason set: {reasons!r}")
    if proof.get("scope_violations") != []:
        return fail("B501D scope violations cannot be excepted")

    verification = proof.get("verification")
    if not isinstance(verification, list) or not verification:
        return fail("B501D verification evidence is missing")
    for result in verification:
        if not isinstance(result, dict) or result.get("state") != "PASS":
            return fail(f"B501D verification is not PASS: {result!r}")

    try:
        for path, expected in B501D_EXPECTED_BLOBS.items():
            actual = git("rev-parse", f"HEAD:{path}")
            if actual != expected:
                return fail(
                    f"B501D candidate artifact blob drift: {path} expected {expected} got {actual}"
                )
    except subprocess.CalledProcessError as exc:
        return fail(f"cannot resolve B501D candidate artifact blob from HEAD: {exc}")

    print("DIFFCIPLINE B501D DEPENDENCY EXCEPTION PASS")
    print(f"base={args.base}")
    print(f"risk={args.risk}")
    print(f"changed_files={len(actual_files)}")
    print(f"added_lines={actual_added}")
...
def main() -> int:
    ...
    # B501D is the exact direct-dependency exposure needed for a later safe
    # Apple freshness-anchor update-only implementation. It is enabled only by
    # the separate canonical B501C gate-hardening merge and pins all four
    # candidate artifacts. It changes no runtime freshness behavior itself.
    if b501d_trusted_base(args.base):
        return check_b501d_exception(args, proof)
