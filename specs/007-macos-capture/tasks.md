...
- [x] C102 Bind 007C grain 2 against live main as the lifecycle-observation Grain: polling `LifecycleWatch` (device-delta + sleep-gap signals, stop-and-join Drop) with pure detectors/mapping onto the untouched 006 contract, zero unsafe, zero new dependencies (block listeners would need the unadopted cidre `blocks` feature; polling the closed cpal enumerator observes the same rows). Live proof here: real-route watch runs clean with no spam; scripted delta proves the row through the thread machinery. Evidence: `b007c2-lifecycle-watch-evidence.md`.
- [x] C103 Bind 007C grain 3 against live main as the sustained-flow Grain: `StreamLossAccount` (saturating counters, wall-clock expectation, settled-window reconciliation), zero unsafe, zero new dependencies. Live proof here: 5 s settled window on the live tap stream with zero stream errors and frames within one in-flight callback of expectation. Evidence: `b007c3-loss-accounting-evidence.md`.
- [x] C104 Record the 007C aggregate evidence in `007c-system-tap-streaming-final-evidence.md`, reconcile `specs/CURRENT.md` with the B007C ledger, and close B007C as CANONICAL_CLOSED in this reconciliation; only then may the Specification 007 closeout reconciliation begin.

## Specification 007 closeout

- [x] Z001 Reconcile the whole 007 lineage (reviewed shaping + the 007C shaping amendment + leaves A/B/C + leaf reconciliations + expected-head merges + post-merge CI/R3) in `007-macos-capture-closeout-final-evidence.md`, update the 007 spec header, and close Specification 007 as `CLOSED_CANONICAL`; only then may Specification 008 shaping begin.
