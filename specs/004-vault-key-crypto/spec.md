...
An intentional restore of an older authenticated backup on a device with a higher anchor must never decrement the anchor. After explicit user-confirmed recovery, Himsat restores verified backup content into a **new epoch greater than the existing anchor** and republishes it as current state with a fresh manifest nonce and hash. When the portable backup generation `G` differs from an existing device's current active generation `H`, the controlling Round 5 contract requires a fail-closed restore rebase before B502: authenticate the complete source state under `G`, authenticate and freeze the stable current state under `H`, copy/re-encrypt restored content into fresh verified target storage under the already-current `H` VRK, construct a non-canonical authenticated staging manifest under `H` at the source epoch, and only then delegate final `anchor + 1` republication to B502. If `G == H`, the recovered and current VRKs must be identical or restore fails. Fresh-device genesis semantics are unchanged.
