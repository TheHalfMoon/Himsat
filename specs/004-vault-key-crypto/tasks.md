...
B101, B102, and B103 code are canonical and exact-post-merge qualified. Historical expected-head transport proof for PR #38 remains not reconstructible post hoc; that evidence gap was repaired forward-only by canonical reconciliation PR #40 without rewriting history. B104 remains blocked until the B103/B104 state/evidence reconciliation below is explicitly expected-head guarded and exact-post-merge qualified.
...
- [ ] B103R001 Canonicalize `b103-secret-protector-final-evidence.md`, this task-ledger reconciliation, and the B104 re-bound `specs/CURRENT.md` state without security-semantic, dependency, provenance, generated-artifact, workflow, donor, or product-runtime changes.
- [ ] B103R002 Merge the exact B103/B104 reconciliation head with an explicit `expected_head_sha` guard and record the actual transport result durably.
- [ ] B103R003 Verify canonical reconciliation parentage and require exact post-merge CI/R3 SUCCESS before any B104 implementation begins.
- [ ] B104 Implement reviewed key hierarchy/domain separation; on lock/rotation/revocation/fatal failure revoke lease first, close handles, release VRK/Recovery KEK/purpose keys, and zeroize owned buffers where reviewed runtime support exists. Actual HKDF-SHA-256 execution and deterministic derivation vectors remain B201; concrete DB/blob post-lock I/O proof remains B105.
