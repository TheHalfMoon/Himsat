...
- [x] B402 Android Keystore adapter with actual app/UID scope, auth policy, invalidation, and hardware capability state. B402A PR #81 qualified the Android-native bridge on Android 17 / API 37 ARM64 emulator with `APP_EXCLUSIVE` package/UID isolation, `NOT_REQUIRED` presence, and observed `SOFTWARE_BACKED` state; guarded canonical merge `d8d73df728d24994e9dfe9677d61e3c158c5db3e` passed post-merge CI `34428219440` and R3 `34428219449`. B402B PR #82 final head `b88c3aee7e674f7981457b2891b356ef23e8b1bb` repaired all three substantive predecessor CodeRabbit findings, passed CI `34491693211` and R3 `34491693214`, merged with observed expected-head protection as `e8aeb1e09591358850b109fdf423a6bfd0b60003`, and passed post-merge CI `34493112106` plus R3 `34493112084`. Stronger presence, TEE/StrongBox, physical-device, freshness, FFI-selection, and Q009 claims remain unmade. Complete evidence is `b402-android-keystore-final-evidence.md`; this checkbox becomes canonical B402 closure only if the B402/B403 reconciliation itself is expected-head merged and post-merge qualified.
- [x] B403 Windows current-user DPAPI/CNG-class adapter reporting `SAME_USER_ACCOUNT`; stronger scope/presence fails unless separately proven. PR #93 accepted head `43c9e702102ec35ca3bfbd64bc3ec70cb87e6e13` passed pull-request CI `34624044015` and R3 `34624043503` on attempt 1 including Windows job `103344703932`, merged through observed `expected_head_sha` protection as canonical `caa815bdcc33262487a7e7587771d9e7c2e9a2cd`, has exact parentage/tree, and passed push-triggered CI `34625185689` plus R3 `34625185699` on attempt 1 including Windows job `103348466854`. Durable post-merge qualification comment: `5637993261`. Complete evidence is `b403-windows-dpapi-final-evidence.md`; this checkbox becomes canonical B403 closure only if the B403/B404 reconciliation itself is expected-head merged and post-merge qualified.
- [x] B404 Linux Secret Service adapter reporting actual scope, minimal non-secret attributes, and no plaintext fallback. PR #97 accepted head `e635972965026037ff86e8b3e209d8dd85bbcfb6` passed pull-request CI `34651506177` and R3 `34651506187` on attempt 1, including genuine Ubuntu 24.04 GNOME Keyring Secret Service job `103434529613` and Windows Rust job `103434529709`; merged through observed `expected_head_sha` protection as canonical `e713487a70a8cbbd53c601b8acc8a2f6379b9e0a` with exact accepted tree. Canonical B404 closure remains conditional on this B404/B405 reconciliation becoming expected-head merged and post-merge qualified. Complete runtime and predecessor evidence is `b404-linux-secret-service-runtime-evidence.md`.
- [x] B405 Mismatched app/vault/generation/policy or unavailable/locked/invalidated protector fails closed and returns no VRK. PR #99 accepted successor head `c82f3579af1d4cf5f0dcb103a54f3b87d0fc0c38` after predecessor head `42667888556287831335ff6f79a27ebb6efc67bd` received a substantive CodeRabbit qualification-scope finding that was repaired forward-only. The accepted head passed pull-request CI `34657287738` and R3 `34657287803` on attempt 1, merged through observed `expected_head_sha` protection as canonical `b01d4e1164a58ee2e640a0bd8c518317c4b00523` with exact accepted tree. Canonical B405 closure remains conditional on this B405/B406 reconciliation becoming expected-head merged and post-merge qualified. Complete evidence is `b405-protector-fail-closed-evidence.md`.
- [x] B406 Prove restart/revocation/unsupported-policy/platform-negative paths where automation permits. PR #101 accepted head `979f597838090b7243ed941c592272eafc16fff1` passed pull-request CI `34660417252` and R3 `34660417268` on attempt 1, merged through observed `expected_head_sha` protection as canonical `f109e416264b43d45787b1532647f08e78f68e0b` with exact accepted tree. Canonical B406 closure remains conditional on this B406/B501 reconciliation becoming expected-head merged and post-merge qualified. Complete evidence is `b406-protector-lifecycle-final-evidence.md`.
